...
class EqualRateValidatorTests(unittest.TestCase):
    def test_equal_rate_clock_metadata_and_weighted_continuity(self):
        ...
        def frame(kind, sequence):
            ...
            payload_bytes = (
                layout.adc_payload_bytes if adc else layout.gpio_payload_bytes
            )
            return equal.Frame(
                kind,
                equal.FLAG_EPOCH_START if sequence == 0 else 0,
                equal.CHECKSUM_ADLER32,
                1,
                sequence,
                0,
                sequence * items * 8,
...
class AuxiliaryPhysicalDevice(SimulatedDevice):
    ...
    def _handle_info(self, request):  # type: ignore[no-untyped-def]
        ...
        if configuration is None:
            ...
            profile = {
                constants.RateProfile.ADC_1MHZ_GPIO_1MHZ: "adc-1mhz-gpio-1mhz",
    ...
    def status(self):  # type: ignore[no-untyped-def]
        ...
        return replace(
            ...
            packet_accounted_frame_skew=abs(
                ...
                // (
                    4
                    if configuration.rate_profile
                    is constants.RateProfile.ADC_1MHZ_GPIO_1MHZ
                    else 1
                )
                - gpio_frames
            ),
...
class PacedAuxSerial:
    ...
    def _pace(self, size: int) -> None:
        ...
        equal = configuration.rate_profile is constants.RateProfile.ADC_1MHZ_GPIO_1MHZ
        if equal:
            interval = coverage_seconds * 4 / (5 if stream_count == 2 else 1)
        now = time.monotonic()
        while self.next_frame_at <= now and len(self.pending) < max(size, 65_536):
            if equal and stream_count == 2:
                self.device._next_stream_index = (
                    0
                    if self.device._adc_first_ticks <= self.device._gpio_first_ticks
                    else 1
                )
...
class AuxiliaryRigFakeDeviceTests(unittest.TestCase):
    def test_temperature_retries_are_bounded_and_failed_samples_retained(self) -> None:
        ...
        for scenario in ("transient", "persistent", "hot"):
            ...
            def exchange(link, kind, *args, calls=calls, scenario=scenario, **kwargs):
            ...
            with (
                ...
                patch.dict(os.environ, {"AUX_INPUT_TEMPERATURE": "1"}),
                patch.object(rig.SerialLink, "exchange", exchange),
            ):
                result = self._run(
                    rig.RUN_CASES["INPUT_COMBINED"],
                    rig.PROFILES[-1],
                ...
                if scenario == "transient":
                    ...
                    self.assertEqual(
                        [2, 0, 0], [sample["status"] for sample in samples]
                    )
                    self.assertEqual(
                        ["before", "before", "after"],
                    ...
                    self.assertIn(
                        "unavailable" if scenario == "persistent" else "safety ceiling",

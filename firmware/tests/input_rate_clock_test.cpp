...
int main() {
  ...
  for (const auto &timing : td::rate_profile::kTimings) {
    ...
    assert(timing.gpio_sample_rate_hz == timing.adc_pair_rate_hz *
           td::input_experiment::kGpioRateMultiplier);
    assert(timing.adc_pair_pit_divider == td::input_experiment::kGpioRateMultiplier);
    for (auto mode : {td::protocol_v2::AuxBankMode::kDisabled,
      ...
      assert(layout.layout.streams[1].coverage_ticks ==
             ...
             (td::input_experiment::kEqualRates ? 4U : 1U));
      ...
      const unsigned weight = td::input_experiment::kEqualRates ? 4U : 1U;

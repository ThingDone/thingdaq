...
The accepted combined-capture validator remains the data-plane authority. This
clock experiment adds fail-closed profile metadata, fixed-cadence bounded
clock/temperature/load samples, reporter-shaped summaries, and distinct
firmware versus infrastructure outcomes. It never retains or emits bulk
capture data. On-chip temperature is not ambient, power, junction-temperature,
reliability, or part-lifetime evidence.
"""
...
SUPPORTED_MODES = frozenset({MODE_SMOKE, MODE_CAMPAIGN})
DEFAULT_SMOKE_CAPTURE_SECONDS = 10.0
DEFAULT_CAMPAIGN_CAPTURE_SECONDS = 600.0
MIN_CAMPAIGN_CAPTURE_SECONDS = 600.0
MAX_SMOKE_CAPTURE_SECONDS = 60.0
DEFAULT_SMOKE_WARMUP_SECONDS = 1.0
DEFAULT_CAMPAIGN_WARMUP_SECONDS = 10.0
DEFAULT_CAPTURE_SECONDS = DEFAULT_SMOKE_CAPTURE_SECONDS
DEFAULT_WARMUP_SECONDS = DEFAULT_SMOKE_WARMUP_SECONDS
...
CLOCK_RESULT_PREFIX = "CLOCK_RESULT "
CLOCK_RESULT_SCHEMA_VERSION = 1
...
TEMPERATURE_INVALID_CALIBRATION = 3
TEMPERATURE_OUT_OF_RANGE = 4
TEMPERATURE_STATUSES = {
    TEMPERATURE_UNAVAILABLE: "UNAVAILABLE",
    TEMPERATURE_VALID: "VALID",
    TEMPERATURE_NOT_READY: "NOT_READY",
    TEMPERATURE_INVALID_CALIBRATION: "INVALID_CALIBRATION",
    TEMPERATURE_OUT_OF_RANGE: "OUT_OF_RANGE",
}
CLOCK_HEALTH_CLOCKS_VALID = 1
CLOCK_HEALTH_TEMPERATURE_VALID = 2
CLOCK_HEALTH_UTILIZATION_VALID = 4
KNOWN_CLOCK_HEALTH_FLAG_MASK = 0x0007
CLOCK_ERROR_CPU_MISMATCH = 1
CLOCK_ERROR_IPG_MISMATCH = 2
CLOCK_ERROR_ADC_MISMATCH = 4
CLOCK_ERROR_PIT_MISMATCH = 8
CLOCK_ERROR_DWT_UNAVAILABLE = 16
CLOCK_ERROR_PHASE_MISMATCH = 32
CLOCK_ERROR_TEMPERATURE_UNAVAILABLE = 64
CLOCK_ERROR_TEMPERATURE_NOT_READY = 128
CLOCK_ERROR_TEMPERATURE_CALIBRATION_INVALID = 256
CLOCK_ERROR_TEMPERATURE_OUT_OF_RANGE = 512
CLOCK_ERROR_UTILIZATION_UNAVAILABLE = 1024
KNOWN_CLOCK_HEALTH_ERROR_MASK = 0x000007FF
CLOCK_ERROR_CLOCK_MASK = 0x0000003F
CLOCK_ERROR_TEMPERATURE_MASK = 0x000003C0
TEMPERATURE_ERROR_BY_STATUS = {
    TEMPERATURE_UNAVAILABLE: CLOCK_ERROR_TEMPERATURE_UNAVAILABLE,
    TEMPERATURE_NOT_READY: CLOCK_ERROR_TEMPERATURE_NOT_READY,
    TEMPERATURE_INVALID_CALIBRATION: CLOCK_ERROR_TEMPERATURE_CALIBRATION_INVALID,
    TEMPERATURE_OUT_OF_RANGE: CLOCK_ERROR_TEMPERATURE_OUT_OF_RANGE,
}
TEMPERATURE_MIN_MILLIDEGREES_CELSIUS = -40_000
TEMPERATURE_MAX_MILLIDEGREES_CELSIUS = 150_000


@dataclass(frozen=True)
class ClockProfileSpec:
    """Exact generated clock/build contract embedded for rig-side grading."""
    ...
    def identity(self) -> dict[str, object]:
        return {
            ...
            "pit_hz": self.pit_hz,
            "dwt_hz": self.dwt_hz,
            "core_voltage_target_mv": self.core_voltage_target_mv,
            "phase_ipg_cycles": self.phase_ipg_cycles,
            "phase_dwt_cycles": self.phase_dwt_cycles,
            "phase_tolerance_dwt_cycles": self.phase_tolerance_dwt_cycles,
        }


CLOCK_PROFILE_SPECS = {
    CLOCK_PROFILE_PRODUCTION_600_MHZ: ClockProfileSpec(
        profile=CLOCK_PROFILE_PRODUCTION_600_MHZ,
        label="PRODUCTION_600_MHZ",
        cpu_profile="600",
        ...
        pit_hz=24_000_000,
        dwt_hz=600_000_000,
        core_voltage_target_mv=1_250,
        phase_ipg_cycles=75,
        phase_dwt_cycles=300,
        phase_tolerance_dwt_cycles=120,
    ),
    CLOCK_PROFILE_EXPERIMENTAL_528_MHZ: ClockProfileSpec(
        profile=CLOCK_PROFILE_EXPERIMENTAL_528_MHZ,
        label="EXPERIMENTAL_528_MHZ",
        cpu_profile="528",
...
def activate_clock_profile(spec: ClockProfileSpec) -> None:
    """Select one exact profile for all reused acquisition grading arithmetic."""

    global _ACTIVE_CLOCK_PROFILE
    global ADC_IPG_CLOCK_HZ
    global ADC_CLOCK_HZ
    global ADC_CALIBRATION_CYCLE_COUNTER_HZ
    global ADC_CALIBRATION_DEADLINE_CYCLES
    global ADC_TRIGGER_DWT_CLOCK_HZ
    global ADC_TRIGGER_IPG_CLOCK_HZ
    global ADC_TRIGGER_INITIAL_DELAYS
    global ADC_TRIGGER_EFFECTIVE_DELAYS
    global ADC_TRIGGER_PHASE_IPG_CYCLES
    global ADC_COMPLETION_EXPECTED_DWT_CYCLES
    global ADC_COMPLETION_TOLERANCE_DWT_CYCLES
    global ADC_TRIGGER_DIAGNOSTIC_DEADLINE_CYCLES

    _ACTIVE_CLOCK_PROFILE = spec
    ADC_IPG_CLOCK_HZ = spec.ipg_hz
    ADC_CLOCK_HZ = spec.adc_hz
    ADC_CALIBRATION_CYCLE_COUNTER_HZ = spec.dwt_hz
    ADC_CALIBRATION_DEADLINE_CYCLES = (
        spec.dwt_hz * ADC_CALIBRATION_DEADLINE_US // 1_000_000
    )
    ADC_TRIGGER_DWT_CLOCK_HZ = spec.dwt_hz
    ADC_TRIGGER_IPG_CLOCK_HZ = spec.ipg_hz
    ADC_TRIGGER_INITIAL_DELAYS = (0, spec.phase_ipg_cycles)
    ADC_TRIGGER_EFFECTIVE_DELAYS = (1, spec.phase_ipg_cycles + 1)
    ADC_TRIGGER_PHASE_IPG_CYCLES = spec.phase_ipg_cycles
    ADC_COMPLETION_EXPECTED_DWT_CYCLES = spec.phase_dwt_cycles
    ADC_COMPLETION_TOLERANCE_DWT_CYCLES = spec.phase_tolerance_dwt_cycles
    ADC_TRIGGER_DIAGNOSTIC_DEADLINE_CYCLES = (
        spec.dwt_hz * ADC_TRIGGER_DIAGNOSTIC_DEADLINE_US // 1_000_000
    )
    ...
    def _validate_payload(frame: Frame) -> None:
        ...
        if frame.kind == INFO_RESPONSE:
            ...
            if (
                ...
                or any(payload[1250:1252])
                or any(payload[1290:1292])
            ):
...
def decode_info(frame: Frame) -> dict[str, object]:
    ...
    info: dict[str, object] = {
        ...
        "clock_profile": payload[376],
        "core_voltage_target_mv": _u16(payload, 378),
        "cpu_clock_hz": _u32(payload, 380),
        "ipg_clock_hz": _u32(payload, 384),
        "profile_adc_clock_hz": _u32(payload, 388),
        "pit_clock_hz": _u32(payload, 392),
        "dwt_clock_hz": _u32(payload, 396),
        "phase_ipg_cycles": _u16(payload, 400),
        "phase_dwt_cycles": _u16(payload, 402),
        "phase_tolerance_dwt_cycles": _u16(payload, 404),
    }
    validate_clock_profile_info(info)
    return info


def validate_clock_profile_info(info: dict[str, object]) -> ClockProfileSpec:
    """Reject unknown or internally contradictory advertised clock metadata."""

    profile_id = _info_int(info, "clock_profile")
    spec = CLOCK_PROFILE_SPECS.get(profile_id)
    if spec is None:
        raise ProtocolFailure(f"INFO advertises unknown clock profile {profile_id}")
    advertised = {
        "core_voltage_target_mv": _info_int(info, "core_voltage_target_mv"),
        "cpu_hz": _info_int(info, "cpu_clock_hz"),
        "ipg_hz": _info_int(info, "ipg_clock_hz"),
        "adc_hz": _info_int(info, "profile_adc_clock_hz"),
        "pit_hz": _info_int(info, "pit_clock_hz"),
        "dwt_hz": _info_int(info, "dwt_clock_hz"),
        "phase_ipg_cycles": _info_int(info, "phase_ipg_cycles"),
        "phase_dwt_cycles": _info_int(info, "phase_dwt_cycles"),
        "phase_tolerance_dwt_cycles": _info_int(info, "phase_tolerance_dwt_cycles"),
    }
    expected = {
    ...
    if advertised != expected:
        raise ProtocolFailure(
            "INFO clock profile metadata contradicts the generated profile: "
            + json.dumps(
                {"advertised": advertised, "expected": expected},
                sort_keys=True,
                separators=(",", ":"),
            )
        )
    activate_clock_profile(spec)
    return spec
...
class ClockHealthSnapshot:
    """One bounded STATUS clock/temperature/load sample."""
    ...
    phase_ipg_cycles: int
    phase_dwt_cycles: int
    runtime_cpu_clock_hz: int
    runtime_ipg_clock_hz: int
    runtime_adc_clock_hz: int
    runtime_pit_clock_hz: int
    runtime_dwt_clock_hz: int
    temperature_millidegrees_celsius: int
    acquisition_service_utilization_basis_points: int
    usb_service_utilization_basis_points: int
    adc_raw_ready_high_water: int
    gpio_raw_ready_high_water: int
    packet_owned_high_water: int
    usb_command_queue_high_water: int
    usb_response_queue_high_water: int
    error_flags: int
    temperature_error_count: int
    clock_mismatch_count: int
    service_counter_error_count: int
    adc_trigger_error_count: int
    adc_hardware_error_count: int

    @property
    def temperature_valid(self) -> bool:
        return bool(self.flags & CLOCK_HEALTH_TEMPERATURE_VALID)

    @property
    def utilization_valid(self) -> bool:
        return bool(self.flags & CLOCK_HEALTH_UTILIZATION_VALID)

    def compact_record(self) -> dict[str, object]:
        return {
            "sample_sequence": self.sample_sequence,
            "device_ticks": self.sample_ticks,
            "clock_profile": self.profile,
            "temperature_status": TEMPERATURE_STATUSES.get(
                self.temperature_status, f"UNKNOWN_{self.temperature_status}"
            ),
            "temperature_millidegrees_celsius": (
                self.temperature_millidegrees_celsius
                if self.temperature_valid
                else None
            ),
            "runtime_clocks_hz": {
                "cpu": self.runtime_cpu_clock_hz,
                "ipg": self.runtime_ipg_clock_hz,
                "adc": self.runtime_adc_clock_hz,
                "pit": self.runtime_pit_clock_hz,
                "dwt": self.runtime_dwt_clock_hz,
            },
            "acquisition_service_utilization_basis_points": (
                self.acquisition_service_utilization_basis_points
                if self.utilization_valid
                else None
            ),
            ...
            "queue_high_water": {
                "adc_raw": self.adc_raw_ready_high_water,
                "gpio_raw": self.gpio_raw_ready_high_water,
                "packet_owned": self.packet_owned_high_water,
                "usb_command": self.usb_command_queue_high_water,
                "usb_response": self.usb_response_queue_high_water,
            },
            "clock_health_flags": self.flags,
            "clock_health_error_flags": self.error_flags,
            "temperature_error_count": self.temperature_error_count,
            "clock_mismatch_count": self.clock_mismatch_count,
            "service_counter_error_count": self.service_counter_error_count,
            "adc_trigger_error_count": self.adc_trigger_error_count,
            "adc_hardware_error_count": self.adc_hardware_error_count,
        }
...
class StatusSnapshot:
    ...
    clock_health: ClockHealthSnapshot
    ...
    def monotonic_counter_items(self) -> tuple[tuple[str, int], ...]:
        counters = {**self.values, **self.extended}
        return tuple(
            (name, counters[name])
...
def decode_status(frame: Frame) -> StatusSnapshot:
    ...
    extended = {
        "packet_owned_depth": _u16(payload, 976),
        "usb_active_frame_size": _u16(payload, 978),
        "adc_packet_filling_depth": _u16(payload, 1072),
        "gpio_packet_filling_depth": _u16(payload, 1074),
    }
    extended.update(_decode_run(payload, 980, "I", _STATUS_CACHE_U32_FIELDS))
    extended.update(_decode_run(payload, 996, "I", _STATUS_PARSER_DETAIL_U32_FIELDS))
    extended.update(_decode_run(payload, 1008, "I", _STATUS_RESPONSE_U32_FIELDS))
    extended.update(_decode_run(payload, 1024, "Q", _STATUS_PRESSURE_U64_FIELDS))
    extended.update(_decode_run(payload, 1040, "Q", _STATUS_EVICTION_U64_FIELDS))
    extended.update(_decode_run(payload, 1076, "Q", _STATUS_DROP_BOUNDARY_U64_FIELDS))
    extended.update(
    ...
    extended.update(_decode_run(payload, 1212, "Q", _STATUS_DROP_PROJECTION_U64_FIELDS))
    clock_health = ClockHealthSnapshot(
        sample_sequence=_u32(payload, 1228),
        sample_ticks=_u64(payload, 1232),
        profile=payload[1240],
        temperature_status=payload[1241],
        flags=_u16(payload, 1242),
        core_voltage_target_mv=_u16(payload, 1244),
        phase_ipg_cycles=_u16(payload, 1246),
        phase_dwt_cycles=_u16(payload, 1248),
        runtime_cpu_clock_hz=_u32(payload, 1252),
        runtime_ipg_clock_hz=_u32(payload, 1256),
        runtime_adc_clock_hz=_u32(payload, 1260),
        runtime_pit_clock_hz=_u32(payload, 1264),
        runtime_dwt_clock_hz=_u32(payload, 1268),
        temperature_millidegrees_celsius=int(
            struct.unpack_from("<i", payload, 1272)[0]
        ),
        acquisition_service_utilization_basis_points=_u16(payload, 1276),
        usb_service_utilization_basis_points=_u16(payload, 1278),
        adc_raw_ready_high_water=_u16(payload, 1280),
        gpio_raw_ready_high_water=_u16(payload, 1282),
        packet_owned_high_water=_u16(payload, 1284),
        usb_command_queue_high_water=_u16(payload, 1286),
        usb_response_queue_high_water=_u16(payload, 1288),
        error_flags=_u32(payload, 1292),
        temperature_error_count=_u32(payload, 1296),
        clock_mismatch_count=_u32(payload, 1300),
        service_counter_error_count=_u32(payload, 1304),
        adc_trigger_error_count=_u32(payload, 1308),
        adc_hardware_error_count=_u32(payload, 1312),
    )
...
def load_fixture_stimulus(raw: str | None) -> FixtureStimulus | None:
    ...
    if value["schema"] != "thingdaq-adc-stimulus-v1":
...
def grade_info(
    ...
    *,
    ...
    expected_clock_profile: int | None,
) -> int:
    ...
    exact = {
        ...
        "clock_profile": _ACTIVE_CLOCK_PROFILE.profile,
        "core_voltage_target_mv": _ACTIVE_CLOCK_PROFILE.core_voltage_target_mv,
        "cpu_clock_hz": _ACTIVE_CLOCK_PROFILE.cpu_hz,
        "ipg_clock_hz": _ACTIVE_CLOCK_PROFILE.ipg_hz,
        "profile_adc_clock_hz": _ACTIVE_CLOCK_PROFILE.adc_hz,
        "pit_clock_hz": _ACTIVE_CLOCK_PROFILE.pit_hz,
        "dwt_clock_hz": _ACTIVE_CLOCK_PROFILE.dwt_hz,
        "phase_ipg_cycles": _ACTIVE_CLOCK_PROFILE.phase_ipg_cycles,
        "phase_dwt_cycles": _ACTIVE_CLOCK_PROFILE.phase_dwt_cycles,
        "phase_tolerance_dwt_cycles": (
            _ACTIVE_CLOCK_PROFILE.phase_tolerance_dwt_cycles
        ),
    }
    ...
    evidence.check(
        "identity.expected_clock_profile",
        (
        ...
        or _ACTIVE_CLOCK_PROFILE.profile == expected_clock_profile,
    )
...
def validate_extended_status_accounting(
    status: StatusSnapshot, *, allow_bounded_stop_tail: bool
) -> None:
    """Validate every post-Phase-08 counter omitted by the reused base model."""
    ...
    if set(extended) != expected_names:
        raise ProtocolFailure("STATUS extended counter classification is incomplete")

    nonzero_errors = {
        name: extended[name]
        for name in _EXTENDED_ZERO_ERROR_FIELDS
        if extended[name]
        and not (
            allow_bounded_stop_tail
            and name
            in {
                "gpio_stop_samples_discarded",
                "adc_raw_gap_pairs",
    ...
    if nonzero_errors:
        raise ProtocolFailure(
            "STATUS extended counters report loss or errors: "
    ...
    for name, limit in gauges.items():
        if not 0 <= extended[name] <= limit:
            raise ProtocolFailure(f"STATUS {name}={extended[name]} exceeds {limit}")
    if extended["usb_active_frame_size"] not in {0, DATA_FRAME_BYTES}:
        raise ProtocolFailure("STATUS USB active frame size is not zero or one frame")
    if status.usb_active_frame_bytes_sent > extended["usb_active_frame_size"]:
        raise ProtocolFailure("STATUS USB active frame progress exceeds its size")
    owned = (
        extended["adc_packet_filling_depth"]
        + extended["gpio_packet_filling_depth"]
        + status.packet_ready_depth
        + status.packet_transmit_depth
    )
    if extended["packet_owned_depth"] != owned:
        raise ProtocolFailure("STATUS packet ownership states do not conserve")
    if extended["packet_owned_depth"] > status.packet_owned_high_water:
        raise ProtocolFailure("STATUS packet ownership exceeds its high water")
    if status.usb_lower_priority_queue_depth != status.packet_transmit_depth:
        raise ProtocolFailure("STATUS USB/data transmit queue depths disagree")

    if extended["packet_pressure_evictions"] != (
        extended["adc_frames_evicted"] + extended["gpio_frames_evicted"]
    ):
        raise ProtocolFailure("STATUS pressure evictions do not reconcile by stream")
    if status.packet_pool_exhaustions != (
        extended["packet_pressure_evictions"]
        + extended["packet_capacity_drops_without_evictable_frame"]
    ):
        raise ProtocolFailure("STATUS pool exhaustion causes do not reconcile")
    for prefix in ("adc", "gpio"):
        evicted = extended[f"{prefix}_frames_evicted"]
        evicted_promoted = extended[f"{prefix}_frames_evicted_after_promotion"]
        dropped_framed = extended[f"{prefix}_frames_dropped_after_framing"]
        dropped_promoted = extended[f"{prefix}_frames_dropped_after_promotion"]
        if not (
            evicted_promoted <= evicted <= dropped_framed
            and evicted_promoted <= dropped_promoted <= dropped_framed
        ):
            raise ProtocolFailure(
                f"STATUS {prefix} eviction/drop boundaries are inconsistent"
            )

    if not (
        extended["responses_completed"]
        <= extended["responses_queued"]
        <= status.commands_accepted
    ):
        raise ProtocolFailure("STATUS response and accepted-command counts disagree")
    if (
        extended["responses_queued"] - extended["responses_completed"]
        > RESPONSE_QUEUE_CAPACITY
    ):
        raise ProtocolFailure("STATUS response completion lag exceeds queue capacity")

    if extended["adc_cache_cpu_invalidations"] != status.adc_buffers_acquired:
        raise ProtocolFailure("STATUS ADC cache invalidations disagree with leases")
    if extended["gpio_cache_cpu_invalidations"] != extended["gpio_buffers_acquired"]:
        raise ProtocolFailure("STATUS GPIO cache invalidations disagree with leases")
    if extended["adc_cache_dma_discards"] < status.adc_buffers_completed:
        raise ProtocolFailure("STATUS ADC DMA cache preparation trails completions")
    if extended["gpio_cache_dma_discards"] < extended["gpio_buffers_completed"]:
        raise ProtocolFailure("STATUS GPIO DMA cache preparation trails completions")

    if not (
        extended["gpio_buffers_completed"]
        >= extended["gpio_buffers_acquired"]
        == extended["gpio_buffers_released"]
    ):
        raise ProtocolFailure("STATUS GPIO buffer stages are inconsistent")
    if extended["gpio_samples_delivered"] != (
        extended["gpio_buffers_acquired"] * GPIO_SAMPLES_PER_FRAME
    ):
        raise ProtocolFailure("STATUS GPIO delivered samples disagree with buffers")
    if extended["gpio_samples_produced"] != (
        extended["gpio_frames_produced"] * GPIO_SAMPLES_PER_FRAME
    ):
        raise ProtocolFailure("STATUS GPIO produced samples disagree with frames")
    if not (
        extended["gpio_frames_produced"]
        >= extended["gpio_frames_packed"]
        >= status.gpio_frames_generated
    ):
        raise ProtocolFailure("STATUS GPIO packer frame stages are inconsistent")

    if extended["adc_pairs_consumed"] != (
        extended["adc_frames_consumed"] * ADC_PAIRS_PER_FRAME
    ):
        raise ProtocolFailure("STATUS ADC consumed pairs disagree with frames")
    if extended["adc_pairs_consumed"] != status.adc_pairs_delivered:
        raise ProtocolFailure("STATUS ADC delivered and consumed pairs disagree")
    if extended["adc_raw_drop_pairs_projected"] > extended["adc_raw_gap_pairs"]:
        raise ProtocolFailure("STATUS ADC projected raw loss exceeds observed gaps")
    if extended["gpio_raw_drop_samples_projected"] > status.gpio_raw_samples_lost:
        raise ProtocolFailure("STATUS GPIO projected raw loss exceeds raw loss")
    if (
        extended["gpio_packer_drop_samples_projected"]
        > status.gpio_packer_samples_dropped
    ):
        raise ProtocolFailure("STATUS GPIO projected packer loss exceeds packer loss")

    if allow_bounded_stop_tail:
        if extended["gpio_stop_samples_discarded"] != status.gpio_raw_samples_lost:
            raise ProtocolFailure("STATUS GPIO STOP-tail counters disagree")
    elif extended["gpio_stop_samples_discarded"]:
        raise ProtocolFailure("running STATUS reports discarded GPIO STOP tail")
...
def validate_status_accounting(
    ...
    validate_extended_status_accounting(
        status, allow_bounded_stop_tail=allow_bounded_stop_tail
    )
...
def robust_numeric_summary(samples: list[int | float]) -> dict[str, object]:
    """Return bounded nearest-rank tails plus the conventional sample median."""
...
class ClockTelemetryCollector:
    """Validate and summarize bounded clock-health samples without raw data."""
    ...
    def __init__(
        ...
        *,
        ...
        self.spec = validate_clock_profile_info(info)
        self.origin = origin
        self.cadence_seconds = cadence_seconds
        self.windows: dict[str, list[dict[str, object]]] = {
            "warmup": [],
            "steady_state": [],
        }
        self.previous: ClockHealthSnapshot | None = None
        self.previous_observed_at: float | None = None
        self.temperature_status_counts = {
            name: 0 for name in TEMPERATURE_STATUSES.values()
        }
        self.utilization_unavailable_samples = 0

    def _validate(self, status: StatusSnapshot) -> None:
        health = status.clock_health
        spec = self.spec
        if health.sample_sequence == 0:
            raise ProtocolFailure("clock-health sample sequence is zero")
        if health.profile != spec.profile:
            raise ProtocolFailure("STATUS clock profile changed from INFO")
        profile_fields = (
        ...
        expected_profile_fields = (
            spec.core_voltage_target_mv,
            spec.phase_ipg_cycles,
            spec.phase_dwt_cycles,
        )
        if profile_fields != expected_profile_fields:
            raise ProtocolFailure(
                "STATUS phase/voltage metadata contradicts the selected profile"
            )
        if health.flags & ~KNOWN_CLOCK_HEALTH_FLAG_MASK:
            raise ProtocolFailure("STATUS clock-health flags contain reserved bits")
        if health.error_flags & ~KNOWN_CLOCK_HEALTH_ERROR_MASK:
            raise ProtocolFailure("STATUS clock-health errors contain reserved bits")
        ...
        if (
            runtime_clocks != expected_clocks
            or not health.flags & CLOCK_HEALTH_CLOCKS_VALID
            or health.error_flags & CLOCK_ERROR_CLOCK_MASK
        ):
            raise ProtocolFailure(
                "runtime clocks contradict the exact advertised profile: "
                + json.dumps(
                    {
                        "actual": runtime_clocks,
                        "expected": expected_clocks,
                        "flags": health.flags,
                        "errors": health.error_flags,
                    },
                    separators=(",", ":"),
                )
            )
        if health.clock_mismatch_count or health.service_counter_error_count:
            raise ProtocolFailure("clock/service health error counters are nonzero")
        if health.adc_trigger_error_count or health.adc_hardware_error_count:
            raise ProtocolFailure("clock-health ADC error counters are nonzero")

        if health.temperature_status not in TEMPERATURE_STATUSES:
            raise ProtocolFailure("STATUS has an unknown temperature state")
        temperature_valid = health.temperature_valid
        if temperature_valid:
            if (
                health.temperature_status != TEMPERATURE_VALID
                or health.error_flags & CLOCK_ERROR_TEMPERATURE_MASK
                or not TEMPERATURE_MIN_MILLIDEGREES_CELSIUS
                <= health.temperature_millidegrees_celsius
                <= TEMPERATURE_MAX_MILLIDEGREES_CELSIUS
            ):
                raise ProtocolFailure("valid temperature metadata is contradictory")
        else:
            required_error = TEMPERATURE_ERROR_BY_STATUS.get(health.temperature_status)
            if (
                health.temperature_status == TEMPERATURE_VALID
                or health.temperature_millidegrees_celsius != 0
                or required_error is None
                or not health.error_flags & required_error
                or health.temperature_error_count == 0
            ):
                raise ProtocolFailure(
                    "unavailable temperature lacks its explicit state/error"
                )

        if health.utilization_valid:
            if (
                health.error_flags & CLOCK_ERROR_UTILIZATION_UNAVAILABLE
                or not 0
                <= health.acquisition_service_utilization_basis_points
                <= 10_000
                or not 0 <= health.usb_service_utilization_basis_points <= 10_000
            ):
                raise ProtocolFailure("valid utilization metadata is contradictory")
        elif (
            health.acquisition_service_utilization_basis_points != 0
            or health.usb_service_utilization_basis_points != 0
            or not health.error_flags & CLOCK_ERROR_UTILIZATION_UNAVAILABLE
        ):
            raise ProtocolFailure(
                "unavailable utilization lacks zero values and an explicit error"
            )
        ...
        for label, health_value, canonical_value, capacity in high_water_checks:
            if not 0 <= health_value <= canonical_value <= capacity:
                raise ProtocolFailure(
                    f"clock-health {label} high water disagrees with STATUS/capacity"
                )

        if self.previous is not None:
            sequence_delta = (
                health.sample_sequence - self.previous.sample_sequence
            ) & 0xFFFFFFFF
            if not 0 < sequence_delta < 0x80000000:
                raise ProtocolFailure("clock-health sample sequence did not advance")
            if health.sample_ticks <= self.previous.sample_ticks:
                raise ProtocolFailure("clock-health sample ticks did not advance")
            monotonic_health_fields = (
            ...
            regressions = {
                name: (getattr(self.previous, name), getattr(health, name))
                for name in monotonic_health_fields
                if getattr(health, name) < getattr(self.previous, name)
            }
            if regressions:
                raise ProtocolFailure(
                    "clock-health counters regressed: "
                    + json.dumps(regressions, sort_keys=True, separators=(",", ":"))
                )
    ...
    def observe(
        ...
        *,
        ...
        if window not in {"preflight", "warmup", "steady_state", "cleanup"}:
            raise ValueError(f"unknown telemetry window {window!r}")
        if (
            self.previous_observed_at is not None
            and observed_at <= self.previous_observed_at
        ):
            raise ProtocolFailure("host monotonic telemetry timestamp did not advance")
        self._validate(status)
        self.previous = status.clock_health
        self.previous_observed_at = observed_at

        health = status.clock_health
        status_name = TEMPERATURE_STATUSES[health.temperature_status]
        self.temperature_status_counts[status_name] += 1
        if not health.utilization_valid:
            self.utilization_unavailable_samples += 1
        if window not in self.windows:
            return
        if sum(len(samples) for samples in self.windows.values()) >= MAX_STATUS_SAMPLES:
            raise ProtocolFailure("clock-health sample storage bound exhausted")
        record = {
            "window": window,
            "monotonic_seconds": observed_at - self.origin,
            **health.compact_record(),
        }
        self.windows[window].append(record)
        emit_event("clock_health_sample", **record)

    def require_complete_windows(self) -> None:
        if not self.windows["warmup"]:
            raise ProtocolFailure("clock comparison captured no warm-up telemetry")
        if len(self.windows["steady_state"]) < 2:
            raise ProtocolFailure(
                "clock comparison needs at least two steady-state telemetry samples"
            )

    @staticmethod
    def _window_summary(samples: list[dict[str, object]]) -> dict[str, object]:
        temperatures = [
            int(sample["temperature_millidegrees_celsius"])
            for sample in samples
            if sample["temperature_millidegrees_celsius"] is not None
        ]
        acquisition_load = [
            int(sample["acquisition_service_utilization_basis_points"])
            for sample in samples
            if sample["acquisition_service_utilization_basis_points"] is not None
        ]
        usb_load = [
            int(sample["usb_service_utilization_basis_points"])
            for sample in samples
            if sample["usb_service_utilization_basis_points"] is not None
        ]
        queue_names = (
            "adc_raw",
            "gpio_raw",
        ...
        queue_maxima = {
            name: max(
                (
                    int(sample["queue_high_water"][name])  # type: ignore[index]
                    for sample in samples
                ),
                default=0,
            )
        ...
        offsets = [float(sample["monotonic_seconds"]) for sample in samples]
        return {
            "sample_count": len(samples),
            "first_monotonic_seconds": offsets[0] if offsets else None,
            "last_monotonic_seconds": offsets[-1] if offsets else None,
            "temperature_millidegrees_celsius": robust_numeric_summary(temperatures),
            "acquisition_service_utilization_basis_points": (
                robust_numeric_summary(acquisition_load)
            ),
            "usb_service_utilization_basis_points": robust_numeric_summary(usb_load),
            "queue_high_water_maxima": queue_maxima,
        }

    def summary(self) -> dict[str, object]:
        warmup = self._window_summary(self.windows["warmup"])
        steady = self._window_summary(self.windows["steady_state"])
        running_offsets = sorted(
            float(sample["monotonic_seconds"])
            for samples in self.windows.values()
            for sample in samples
        )
        observed_intervals = [right - left for left, right in pairwise(running_offsets)]
        steady_temperature = steady["temperature_millidegrees_celsius"]
        steady_acquisition = steady["acquisition_service_utilization_basis_points"]
        steady_usb = steady["usb_service_utilization_basis_points"]
        assert isinstance(steady_temperature, dict)
        assert isinstance(steady_acquisition, dict)
        assert isinstance(steady_usb, dict)
        thermal_ready = steady_temperature["count"] == steady["sample_count"]
        utilization_ready = (
            steady_acquisition["count"] == steady["sample_count"]
            and steady_usb["count"] == steady["sample_count"]
        )
        return {
            "profile": self.spec.identity(),
            "cadence_seconds": self.cadence_seconds,
            "observed_cadence_seconds": robust_numeric_summary(observed_intervals),
            "windows": {"warmup": warmup, "steady_state": steady},
            "temperature_status_counts": self.temperature_status_counts,
            "utilization_unavailable_samples": self.utilization_unavailable_samples,
            "thermal_evidence": "READY" if thermal_ready else "INCONCLUSIVE",
            "utilization_evidence": "READY" if utilization_ready else "INCONCLUSIVE",
            "limitations": [
                "Temperature is the Teensy on-chip sensor, not measured ambient, power, or junction temperature.",
                "No reliability, lifetime, or part-aging inference is supported.",
                "Core voltage is an advertised profile target, not a measured rail value.",
                "Analog aperture and external GPIO pad propagation are outside this run.",
            ],
        }
...
def reconcile_final_status(
    ...
    *,
    ...
    extended_exact = {
        ...
        "adc_cache_cpu_invalidations": adc_frames,
        "gpio_cache_cpu_invalidations": gpio_frames,
        "bad_flags": 0,
        "bad_payloads": 0,
        "bad_request_ids": 0,
        "response_queue_rejections": 0,
        "response_reservations_abandoned": 0,
        "packet_pressure_evictions": 0,
        "packet_capacity_drops_without_evictable_frame": 0,
        "adc_frames_evicted": 0,
        "adc_frames_evicted_after_promotion": 0,
        "gpio_frames_evicted": 0,
        "gpio_frames_evicted_after_promotion": 0,
        "adc_frames_dropped_after_framing": 0,
        "adc_frames_dropped_after_promotion": 0,
        "gpio_frames_dropped_after_framing": 0,
        "gpio_frames_dropped_after_promotion": 0,
        ...
        "gpio_stop_samples_discarded": status.gpio_raw_samples_lost,
        "gpio_frames_produced": gpio_frames,
        "gpio_samples_produced": gpio_items,
        "gpio_frames_packed": gpio_frames,
        "gpio_duplicate_samples_ignored": 0,
        "adc_frames_consumed": adc_frames,
        "adc_pairs_consumed": adc_items,
        "adc_raw_gap_pairs": 0,
        "adc_raw_drop_pairs_projected": 0,
        "gpio_raw_drop_samples_projected": 0,
        "gpio_packer_drop_samples_projected": 0,
    }
    extended_bounded = {
        "adc_cache_dma_discards": (adc_frames, 0xFFFFFFFF),
        "gpio_cache_dma_discards": (gpio_frames, 0xFFFFFFFF),
        "responses_queued": (0, expected_commands),
        "responses_completed": (0, expected_commands),
    }
    extended_classified = set(extended_exact) | set(extended_bounded)
    evidence.equal(
        "final.all_extended_status_fields_classified",
        sorted(status.extended),
        sorted(extended_classified),
    )
    for name, expected in extended_exact.items():
        evidence.equal(
            f"final.extended_counter.{name}", expected, status.extended[name]
        )
    for name, (minimum, maximum) in extended_bounded.items():
        value = status.extended[name]
        evidence.check(
            f"final.extended_counter.{name}",
            {"minimum": minimum, "maximum": maximum},
            value,
            minimum <= value <= maximum,
        )
...
def exact_run_identity(info: dict[str, object]) -> dict[str, object]:
    """Return stable hardware, firmware, build, protocol, and profile identity."""

    spec = validate_clock_profile_info(info)
    return {
        "hardware_serial": _info_int(info, "hardware_serial"),
        "firmware_version": list(info["firmware_version"]),
        "firmware_build_id": info["build_id"],
        "board_id": _info_int(info, "board_id"),
        "mcu_id": _info_int(info, "mcu_id"),
        "protocol_version": _info_int(info, "protocol_version"),
        "clock_profile": spec.identity(),
    }


def classify_runtime_error(error: BaseException) -> str:
    """Separate unavailable rig/service evidence from observed firmware faults."""

    if isinstance(error, (DeadlineExpired, OSError)):
        return "infrastructure"
    if type(error).__name__ in {"SerialException", "SerialTimeoutException"}:
        return "infrastructure"
...
def run_acceptance(
    ...
    *,
    ...
    expected_clock_profile: int | None = None,
    ...
    telemetry: ClockTelemetryCollector | None = None
    ...
    try:
        ...
        identity = exact_run_identity(info)
        telemetry = ClockTelemetryCollector(
            info,
            origin=run_origin,
            cadence_seconds=status_interval_seconds,
        )
        evidence.equal(
        ...
        resolution_bits = grade_info(
            ...
            expected_clock_profile=expected_clock_profile,
        )
        ...
        while True:
            ...
            if now >= next_status_at:
                ...
                telemetry.observe(
                    status,
                    window=("warmup" if timed_started_at is None else "steady_state"),
                    observed_at=time.monotonic(),
                )
        ...
        cleanup_report["stop_attempted"] = True
        ...
        cleanup_report["stop_succeeded"] = stop_frame.payload[4] == STATE_IDLE
        ...
        cleanup_report["idle_confirmed"] = final_status.device_state == STATE_IDLE
        ...
        telemetry.require_complete_windows()
        run_summary = {
            "run_id": validator.run_id,
            "capture_elapsed_seconds": capture_elapsed,
            "warmup_seconds": warmup_seconds,
            "steady_state_seconds_requested": capture_seconds,
            "status_interval_seconds": status_interval_seconds,
            "status_sample_count": status_count,
            "status_latency_seconds": robust_numeric_summary(status_latencies),
            "command_latency_seconds": robust_numeric_summary(all_command_latencies),
            "timed_adc_frames": timed_adc_frames,
            "timed_gpio_frames": timed_gpio_frames,
            "timed_adc_pairs": timed_adc_items,
            "timed_gpio_samples": timed_gpio_items,
            "adc_pair_rate_hz": timed_adc_items / capture_elapsed,
            "gpio_sample_rate_hz": timed_gpio_items / capture_elapsed,
            ...
            "maximum_receive_gap_seconds": (validator.maximum_receive_gap_seconds),
            "final_error_counters": {
                **{name: final_status.values[name] for name in _ZERO_ERROR_FIELDS},
                **{
                    name: final_status.extended[name]
                    for name in _EXTENDED_ZERO_ERROR_FIELDS
                },
                "clock_health_error_flags": (final_status.clock_health.error_flags),
                "clock_mismatch_count": (
                    final_status.clock_health.clock_mismatch_count
                ),
                "service_counter_error_count": (
                    final_status.clock_health.service_counter_error_count
                ),
                "health_adc_trigger_error_count": (
                    final_status.clock_health.adc_trigger_error_count
                ),
                "health_adc_hardware_error_count": (
                    final_status.clock_health.adc_hardware_error_count
                ),
            },
        }
        emit_event(
            "final_firmware_counters",
            counters={**final_status.values, **final_status.extended},
            clock_health=final_status.clock_health.compact_record(),
        )
        completed = not evidence.failures
    except Exception as error:  # noqa: BLE001 - stdout is remote diagnosis
        message = f"{type(error).__name__}: {error}"
        failure_class = classify_runtime_error(error)
        ...
        if not completed:
            ...
            try:
                ...
                cleanup_report["stop_succeeded"] = cleanup.payload[4] == STATE_IDLE
                emit_event(
                    "cleanup_stop",
                ...
                idle_gauges = {
                    "packet_ready_depth": idle_status.packet_ready_depth,
                    "packet_transmit_depth": idle_status.packet_transmit_depth,
                    "packet_owned_depth": idle_status.packet_owned_depth,
                    "adc_packet_filling_depth": (idle_status.adc_packet_filling_depth),
                    "gpio_packet_filling_depth": (
                        idle_status.gpio_packet_filling_depth
                    ),
                    "usb_command_queue_depth": idle_status.usb_command_queue_depth,
                    "usb_response_queue_depth": (idle_status.usb_response_queue_depth),
                    "usb_lower_priority_queue_depth": (
                        idle_status.usb_lower_priority_queue_depth
                    ),
                    "usb_active_frame_bytes_sent": (
                        idle_status.usb_active_frame_bytes_sent
                    ),
                    "usb_active_frame_size": idle_status.usb_active_frame_size,
                }
                cleanup_report["idle_confirmed"] = (
                    idle_status.device_state == STATE_IDLE
                    and not any(idle_gauges.values())
                )
                cleanup_report["idle_gauges"] = idle_gauges
                if not cleanup_report["idle_confirmed"]:
                    raise ProtocolFailure(
                        "best-effort cleanup did not confirm an empty IDLE state"
                    )
                ...
                cleanup_report["error"] = cleanup_message
                emit_event(
                    "cleanup_stop_failed",
                    error=cleanup_message,
                )
                evidence.failures.append("cleanup: " + cleanup_message)
                if failure_class is None:
                    failure_class = classify_runtime_error(cleanup_error)
                    failure_reason = cleanup_message
        if evidence.failures and failure_class is None:
            failure_class = "firmware"
            failure_reason = evidence.failures[0]
    return AcceptanceResult(
        ...
        digital_mode=digital_mode,
        identity=identity,
        telemetry=(telemetry.summary() if telemetry is not None else {}),
...
def _clock_profile_environment(name: str) -> int | None:
    raw = os.environ.get(name)
    if raw is None:
        return None
    normalized = raw.strip().upper().replace("-", "_")
    aliases = {
        "0": CLOCK_PROFILE_PRODUCTION_600_MHZ,
        "600": CLOCK_PROFILE_PRODUCTION_600_MHZ,
        "600MHZ": CLOCK_PROFILE_PRODUCTION_600_MHZ,
        "PRODUCTION_600_MHZ": CLOCK_PROFILE_PRODUCTION_600_MHZ,
        "1": CLOCK_PROFILE_EXPERIMENTAL_528_MHZ,
...
def _optional_text_environment(name: str, *, maximum_length: int = 128) -> str | None:
    raw = os.environ.get(name)
    if raw is None:
        return None
    if (
...
def _selected_environment(primary: str, legacy: str) -> str:
    return primary if primary in os.environ else legacy


def emit_clock_result(payload: dict[str, object]) -> None:
    print(
        CLOCK_RESULT_PREFIX + json.dumps(payload, sort_keys=True, separators=(",", ":"))
    )
...
def main() -> int:
    ...
    if not port_name:
        ...
        emit_clock_result(
            {
                "schema_version": CLOCK_RESULT_SCHEMA_VERSION,
                "kind": "thingdaq-clock-comparison-run",
                "mode": None,
                "result": "NOT_RUN",
                "reason": reason,
                "failure_class": "configuration",
                "performance_result": "NOT_RUN",
                "thermal_result": "NOT_RUN",
                "identity": None,
                "run": {},
                "telemetry": {},
                "cleanup": {
        ...
        return 2
    try:
        mode = os.environ.get("CLOCK_COMPARISON_MODE", MODE_SMOKE).strip().lower()
        if mode not in SUPPORTED_MODES:
            raise ValueError("CLOCK_COMPARISON_MODE must be exactly smoke or campaign")
        default_capture = (
            DEFAULT_SMOKE_CAPTURE_SECONDS
            if mode == MODE_SMOKE
            else DEFAULT_CAMPAIGN_CAPTURE_SECONDS
        )
        default_warmup = (
            DEFAULT_SMOKE_WARMUP_SECONDS
            if mode == MODE_SMOKE
            else DEFAULT_CAMPAIGN_WARMUP_SECONDS
        )
        capture_seconds = _positive_float_environment(
            _selected_environment("CLOCK_CAPTURE_SECONDS", "COMBINED_CAPTURE_SECONDS"),
            default_capture,
        )
        warmup_seconds = _nonnegative_float_environment(
            _selected_environment("CLOCK_WARMUP_SECONDS", "COMBINED_WARMUP_SECONDS"),
            default_warmup,
        )
        status_interval_seconds = _positive_float_environment(
            _selected_environment(
                "CLOCK_STATUS_INTERVAL_SECONDS",
                "COMBINED_STATUS_INTERVAL_SECONDS",
            ),
            DEFAULT_STATUS_INTERVAL_SECONDS,
        )
        checksum_algorithm = _checksum_environment(
            _selected_environment(
                "CLOCK_CHECKSUM_ALGORITHM", "COMBINED_CHECKSUM_ALGORITHM"
            ),
        ...
        if mode == MODE_CAMPAIGN and capture_seconds < MIN_CAMPAIGN_CAPTURE_SECONDS:
            raise ValueError(
                "campaign capture must contain at least 600 steady-state seconds"
            )
        if status_interval_seconds > MAX_STATUS_INTERVAL_SECONDS:
            raise ValueError(
                "clock telemetry cadence must be no slower than "
                f"{MAX_STATUS_INTERVAL_SECONDS} seconds"
            )
        if warmup_seconds < status_interval_seconds:
            raise ValueError(
                "warm-up must span at least one complete telemetry cadence"
            )
        expected_hardware_serial = _optional_uint32_environment(
            "EXPECTED_HARDWARE_SERIAL"
        )
        expected_clock_profile = _clock_profile_environment("EXPECTED_CLOCK_PROFILE")
        expected_fqbn = os.environ.get("EXPECTED_FQBN")
        if expected_fqbn is not None:
            matching_profiles = [
                spec.profile
                for spec in CLOCK_PROFILE_SPECS.values()
            ...
            if (
                ...
                and expected_clock_profile != fqbn_profile
            ):
                raise ValueError("EXPECTED_FQBN contradicts EXPECTED_CLOCK_PROFILE")
            expected_clock_profile = fqbn_profile
        artifact_sha256 = _optional_sha256_environment(
        ...
        emit_clock_result(
            {
                "schema_version": CLOCK_RESULT_SCHEMA_VERSION,
                "kind": "thingdaq-clock-comparison-run",
                "mode": os.environ.get("CLOCK_COMPARISON_MODE", MODE_SMOKE),
                "result": "NOT_RUN",
                "reason": reason,
                "failure_class": "configuration",
                "performance_result": "NOT_RUN",
    ...
    try:
        ...
        emit_event("open_failed", error=reason)
        emit_clock_result(
            {
                "schema_version": CLOCK_RESULT_SCHEMA_VERSION,
                "kind": "thingdaq-clock-comparison-run",
                ...
                "performance_result": "INCONCLUSIVE",
                "thermal_result": "INCONCLUSIVE",
                "identity": None,
                "declared_identity": {
                    "firmware_artifact_sha256": artifact_sha256,
                    "source_id": source_id,
                    "job_id": job_id,
                    "fixture_declaration_sha256": fixture_declaration_sha256,
                },
                "run": {},
                "telemetry": {},
                "cleanup": {
    ...
    try:
        ...
        try:
            port.close()
        except Exception as error:  # noqa: BLE001 - rig-visible close failure
            close_error = f"{type(error).__name__}: {error}"
            emit_event("close_failed", error=close_error)

    failures = list(result.evidence.failures)
    failure_class = result.failure_class
    failure_reason = result.failure_reason
    if close_error is not None:
        failures.append("serial close: " + close_error)
        if failure_class is None:
            failure_class = "infrastructure"
            failure_reason = close_error

    identity = dict(result.identity or {})
    profile_identity = identity.get("clock_profile")
    if expected_fqbn is not None and (
        not isinstance(profile_identity, dict)
        or profile_identity.get("fqbn") != expected_fqbn
    ):
        message = "runtime profile FQBN contradicts EXPECTED_FQBN"
    ...
    if failures:
        performance_result = "FAIL" if failure_class == "firmware" else "INCONCLUSIVE"
        overall_result = performance_result
        reason = failure_reason or failures[0]
    elif thermal_evidence != "READY" or utilization_evidence != "READY":
        performance_result = "PASS"
        overall_result = "INCONCLUSIVE"
        failure_class = "telemetry"
        missing = []
        if thermal_evidence != "READY":
            missing.append("steady-state temperature")
        if utilization_evidence != "READY":
            missing.append("steady-state utilization")
        reason = "missing valid " + " and ".join(missing) + " evidence"
    else:
        performance_result = "PASS"
        overall_result = "PASS"
        failure_class = None
    ...
    summary = {
        "schema_version": CLOCK_RESULT_SCHEMA_VERSION,
        "kind": "thingdaq-clock-comparison-run",
        "validation_semantics": "physical-combined-clock-comparison-v1",
        "mode": mode,
        ...
        "thermal_result": (
            "EVIDENCE_READY" if thermal_evidence == "READY" else "INCONCLUSIVE"
        ),
        "utilization_result": (
            "EVIDENCE_READY" if utilization_evidence == "READY" else "INCONCLUSIVE"
        ),
        ...
        "stimulus_scope": {
            ...
            "analog_quality": (
                "GRADED" if result.analog_quality_graded else "NOT_GRADED"
            ),
            "analog_aperture": "NOT_GRADED",
    ...
    emit_clock_result(summary)
    if overall_result == "PASS":
        return 0
    if failure_class == "firmware":
        return 1
    if failure_class == "telemetry":
        return 3
    return 2

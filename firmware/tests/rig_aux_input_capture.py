...
RELEASE_FIXED_1MHZ = os.environ.get("AUX_INPUT_RELEASE_FIXED_1MHZ", "0") == "1"
...
EXPECTED_CAPABILITIES = 0x000006FF if RELEASE_FIXED_1MHZ else 0x000007FF
...
PROFILES = tuple(
    RateProfile(
        ...
        profile.adc_period_ticks
        if EXPERIMENT_EQUAL_RATES
        else profile.gpio_period_ticks,
        profile.pit_divider
        * (4 if EXPERIMENT_EQUAL_RATES and profile.value != 4 else 1),
        profile.pit_divider
        * (4 if EXPERIMENT_EQUAL_RATES and profile.value != 4 else 1)
        - 1,
...
class FrameParser:
    ...
    def _validate_payload(frame: Frame) -> None:
        ...
        if frame.kind == GET_TEMPERATURE_RESPONSE:
            sensor, reserved1, reserved2, value = struct.unpack_from(
                "<BBHi", payload, 4
            )
...
def decode_info(frame: Frame) -> dict[str, object]:
    ...
    return {
        ...
        "rate_profiles": tuple(_decode_rate_profile(p, 440 + 48 * i) for i in range(5)),
    }
...
def grade_static_info(
    ...
    *,
    ...
    exact: dict[str, object] = {
        ...
        "supported_rate_profile_mask": 0x10 if RELEASE_FIXED_1MHZ else 0x1F,
        "supported_aux_mode_mask": 0x03,
        "aux_gpio_pin_count": 8,
        "rate_profile_count": 5,
...
def validate_status(
    ...
    *,
    ...
    reported_profile = PROFILES[4 if RELEASE_FIXED_1MHZ else 0] if final else profile
    ...
    if adc_enabled:
        for name, expected in {
            ...
            if status.values[name] != expected:
                raise ProtocolFailure(
                    f"{name}: expected {expected}, got {status.values[name]}"
                )
    ...
    *,
    ...
    def sample_temperature(phase: str) -> None:
        ...
        for attempt in range(5):
            ...
            if sensor != 0:
                raise ProtocolFailure(
                    f"temperature reading unavailable: status={sensor}"
                )
            # Conservative campaign abort, not a change to core thermal protection.
            if value >= 80000:
                raise ProtocolFailure(
                    f"temperature safety ceiling reached: {value / 1000} C"
                )
            return

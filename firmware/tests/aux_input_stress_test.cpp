...
void testEveryProfileAndSchedulerFailureBoundary() {
  ...
  for (std::uint32_t raw = 0U; raw < 256U; ++raw) {
    ...
    if (raw == 4U) {
      expect(result.ok() && result.schedule.adc_pair_rate_hz == 1000000U &&
                 result.schedule.gpio_sample_rate_hz == 1000000U &&
                 result.schedule.adc_pair_pit_divider == 1U,
             "equal-rate release profile must derive a one-to-one schedule");
      continue;
    }

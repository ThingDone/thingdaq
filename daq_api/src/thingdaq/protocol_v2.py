"""Experimental protocol-v2 auxiliary-input envelope and bounded parser.

The public acquisition models remain protocol-v1-first in this prototype
stage.  This module supplies the strict wire codec, INFO/configuration
validation, request/echo correlation, and explicit version selection that the
typed auxiliary-input API layers on next.
"""
...
def _rate_profile_for_pair(adc_rate: int, gpio_rate: int) -> constants.RateProfile:
    ...
    if gpio_rate != 4 * adc_rate:
        raise V2FrameValidationError(
            "GPIO rate must be exactly four times the ADC-pair rate",
    ...
    for profile, timing in constants.RATE_PROFILE_TIMING.items():
        if (
            ...
            return profile
    raise V2FrameValidationError(
        "rate pair is not one of the four exact profiles",
...
def _validate_info_payload(payload: bytes) -> None:
    ...
    if payload[constants.INFO_RESPONSE_SUPPORTED_RATE_PROFILE_MASK_OFFSET] != (
        constants.SUPPORTED_RATE_PROFILE_MASK
    ):

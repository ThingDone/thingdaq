...
class RateProfileTiming:
    ...
    def from_profile(cls, profile: RateProfile | int) -> RateProfileTiming:
        ...
        try:
            ...
            raise ValueError(
                "rate profile is not one of the five exact profiles"
            ) from exc
    ...
    def at_cpu_clock(self, clock_hz: int) -> RateProfileTiming:
        if clock_hz not in (450_000_000, 600_000_000):
            raise ValueError("unsupported acquisition core clock")
        return dataclass_replace(
            self,
            completion_expected_dwt_cycles=(clock_hz // (2 * self.adc_pair_rate_hz)),
        )
...
class AdcTriggerMetadata:
    ...
    def __post_init__(self) -> None:
        ...
        try:
            ...
            ).at_cpu_clock(self.dwt_clock_hz)
        ...
        fixed_scalars = (
            ...
            ("dwt_clock_hz", self.dwt_clock_hz),
...
class AdcBlockMetadata:
    ...
    @lru_cache(maxsize=32, typed=True)
    def for_rate_profile(
        cls, profile: RateProfile, source: constants.Source
    ) -> AdcBlockMetadata:
        """Reuse immutable default metadata for decoded frames, not live evidence."""
        timing = RateProfileTiming.from_profile(profile)
        return cls(
            source=source,
            pair_rate_hz=timing.adc_pair_rate_hz,
            ...
            trigger=AdcTriggerMetadata.for_rate_profile(profile),
        )
...
class DAQConfiguration:
    ...
    def __post_init__(self) -> None:
        ...
        if stream_mask == constants.StreamMask.NONE and (
            ...
            or rate_profile
            not in (v2_constants.DEFAULT_RATE_PROFILE, RateProfile.ADC_1MHZ_GPIO_1MHZ)
        ):
...
class ChecksumBenchmarkResult:
    ...
    def __post_init__(self) -> None:
        ...
        if (
            ...
            or self.cycle_counter_hz not in (450_000_000, 600_000_000)
...
class GpioClockDiagnosticResult:
    ...
    def __post_init__(self) -> None:
        ...
        expected_scheduled = (
            self.dwt_elapsed_cycles // (self.dwt_counter_hz // self.request.rate_hz)
            if self.dwt_counter_hz in (450_000_000, 600_000_000)
            else 0
        )
        if (
            ...
            or (
                self.dwt_counter_hz in (450_000_000, 600_000_000)
                and self.scheduled_event_count != expected_scheduled
            )
            or (
                not errors
                and (
                    self.dwt_counter_hz not in (450_000_000, 600_000_000)
...
class AuxiliaryInputMetadata:
    ...
    supported_rate_profile_mask: int = v2_constants.SUPPORTED_RATE_PROFILE_MASK
    dwt_clock_hz: int = constants.ADC_TRIGGER_DWT_CLOCK_HZ
    ...
    def __post_init__(self) -> None:
        ...
        expected_profiles = tuple(
            t.at_cpu_clock(self.dwt_clock_hz) for t in RATE_PROFILE_TIMINGS
        )
        if profiles != expected_profiles or modes != tuple(AuxBankMode):
            raise ValueError("auxiliary INFO omits or reorders a generated profile")
        if (
            not self.supported_rate_profile_mask
            or self.supported_rate_profile_mask
            & ~v2_constants.SUPPORTED_RATE_PROFILE_MASK
            or not self.supported_rate_profile_mask & (1 << int(selected_profile))
        ):
            raise ValueError("auxiliary INFO rate support mask is invalid")
    ...
    def selected_timing(self) -> RateProfileTiming:
        return RateProfileTiming.from_profile(self.selected_rate_profile).at_cpu_clock(
            self.dwt_clock_hz
        )
    ...
    def supports_configuration(self, configuration: DAQConfiguration) -> bool:
        return (
            bool(
                self.supported_rate_profile_mask
                & (1 << int(configuration.rate_profile))
            )
            and configuration.aux_bank_mode in self.supported_aux_bank_modes
            and any(
...
class DeviceCapabilities:
    ...
    def supports_configuration(self, configuration: DAQConfiguration) -> bool:
        ...
        if not self.supported_configuration_mask & configuration.profile:
            return False
        if self.auxiliary is not None:
            return self.auxiliary.supports_configuration(configuration)
...
def _pack_auxiliary_info(
    ...
    c = v2_constants
    payload[c.INFO_RESPONSE_SUPPORTED_RATE_PROFILE_MASK_OFFSET] = (
        auxiliary.supported_rate_profile_mask
    )
...
def _unpack_auxiliary_info(payload: bytes) -> AuxiliaryInputMetadata:
    ...
    if (
        not payload[c.INFO_RESPONSE_SUPPORTED_RATE_PROFILE_MASK_OFFSET]
        or payload[c.INFO_RESPONSE_SUPPORTED_RATE_PROFILE_MASK_OFFSET]
    ...
    try:
        return AuxiliaryInputMetadata(
            ...
            supported_rate_profile_mask=byte(
                c.INFO_RESPONSE_SUPPORTED_RATE_PROFILE_MASK_OFFSET
            ),
            dwt_clock_hz=u32_at(c.INFO_RESPONSE_ADC_TRIGGER_DWT_CLOCK_HZ_OFFSET),
...
class ADCBlock:
    ...
    def __post_init__(self) -> None:
        ...
        if (
            not adc_payload_codes_valid(payload)
            if self.metadata.code_min == 0 and self.metadata.code_max == 4095
            else any(
    ...
    def from_v2_frame(
        ...
        return cls(
            ...
            metadata=AdcBlockMetadata.for_rate_profile(
                configuration.rate_profile, source
            ),

...
class DeviceCommandError(ThingDAQError):
    ...
    def __init__(
        self,
        command: constants.FrameKind | v2_constants.FrameKind,
...
class ThingDAQ:
    ...
    def __init__(
        ...
        *,
        ...
        self._closed = False
        self._adc_metadata_cache: (
            tuple[DeviceInfo, Status | None, constants.Source, AdcBlockMetadata] | None
        ) = None
        self._reader.start()
        ...
        *,
        ...
        with self._lock:
            ...
            if configuration is None:
                ...
                auxiliary = capabilities.auxiliary if capabilities is not None else None
                ...
                if rate_profile is not None:
                    ...
                    matching_profiles = tuple(
                        ...
                        if (
                            auxiliary is None
                            or auxiliary.supported_rate_profile_mask
                            & (1 << int(timing.profile))
                        )
                        and (
    ...
    def get_temperature(self) -> TemperatureReading:
        ...
        with self._lock:
            ...
            if not isinstance(response.value, TemperatureReading):
                raise UnexpectedMessageError(
                    "temperature response has no sensor reading"
                )
            return response.value
    ...
    def gpio_clock_diagnostic(
        ...
        *,
        ...
        if rate_hz is None:
            rate_hz = (
                self._device_info.gpio_sample_rate_hz
                if self._device_info is not None
                else constants.GPIO_CLOCK_PRODUCTION_RATE_HZ
            )
    ...
    def stop(self) -> constants.DeviceState:
        ...
        with self._lock:
            ...
            self._configuration = None
            assert self._verified_identity is not None
            self._wire_protocol_version = self._verified_identity.protocol_version
            self._run_id = response.run_id
    ...
    def _read_info(
        ...
        *,
        ...
        try:
            ...
            if (
                selected_version != constants.PROTOCOL_VERSION
                or error.error_code != constants.ErrorCode.UNSUPPORTED_VERSION
            ):
                raise
            selected_version = v2_constants.PROTOCOL_VERSION
            response = self._command(
                constants.FrameKind.INFO_REQUEST, protocol_version=selected_version
            )
        ...
        if selected_version == constants.PROTOCOL_VERSION:
            ...
            if self._verified_identity is not None:
                identity = replace(
                    identity, protocol_version=self._verified_identity.protocol_version
                )
    ...
    def _process_block(self, block: DataBlock) -> StreamItem:
        ...
        if isinstance(block, ADCBlock):
            ...
            status = self._last_status
            try:
                cached = self._adc_metadata_cache
                if (
                    cached is None
                    or cached[0] is not info
                    or cached[1] is not status
                    or cached[2] is not configuration.source
                ):
                    ...
                    cached = (
                        ...
                        info.adc_block_metadata(
                            configuration.source, acquisition=acquisition
                        ),
                    )
                    self._adc_metadata_cache = cached
                block = replace(
                    block,
                    metadata=cached[3],
                    gap=None,

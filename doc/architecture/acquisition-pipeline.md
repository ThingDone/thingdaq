...
24 MHz PERCLK
    |
    +-> PIT0, LDVAL=23, 1 MHz master
         |
         +-> XBARA1 input 56 -> output 0 -> DMAMUX 30 -> eDMA 2
         |                                           -> GPIO2_PSR ring
         |
         +-> chained PIT1, LDVAL=0, 1 MHz pair event
              |
...
| `gpio_processing_cpu_basis_points` | hundredths of one percent of the reported core clock (450 MHz in 1.1.0) |
...
self-contained rig are complete. Exact pre-rename build
`tdaq-25b8d210adb6e4ac` passed a

...
The exact production 600 MHz and experimental 528 MHz no-upload builds passed deterministic local validation with a common firmware source identity, identical allocatable target layout, identical memory use, and unchanged nominal 1 MHz ADC-pair and 4 MHz GPIO schedules. The rig, performance-compatibility, and thermal-benefit levels remain **INCONCLUSIVE** because the Teensy did not enumerate during either bounded valid 600 MHz smoke attempt.
...
### Physical campaign disposition

Submission `3828ce14-97f8-436b-a979-03ea0a634316` was excluded before power-on because an option-qualified FQBN is not a service target. The corrected 600 MHz smoke submissions `1debfc21-6999-494f-9c36-1995990e8f36` and `b65cacb4-1161-424f-a03c-1ad38b85c272` reached bounded power-on but the board did not enumerate within 20 seconds. The service recovered to healthy, normal, queue-depth-zero state after each attempt. The prerequisite 600 MHz smoke therefore remained inconclusive; the 528 MHz smoke and planned 600/528/528/600 ABBA endurance sequence were not run. No physical rate, loss, latency, service-load, queue, temperature-trace, or temperature-statistic value was observed or inferred.
...
Reason: The exact 600 MHz and 528 MHz profiles passed the complete local gate, but the comparison board did not enumerate in either bounded valid 600 MHz smoke attempt, so no physical performance or thermal comparison was possible.
...
| physical-campaign | **rig** | INCONCLUSIVE | Credential-safe service preflight followed by strictly sequential bounded smoke submissions, with one infrastructure retry and automatic service recovery checks; later stages were not submitted after the prerequisite smoke remained inconclusive. | `python3 firmware/tests/rig_clock_comparison.py` |
...
| lifecycle_complete | INFO, CONFIGURE, START, bounded data capture, STATUS, STOP, and cleanup complete in the declared order. | **INCONCLUSIVE** | true | {"abba_temperature_statistics":null,"abba_temperature_trace_millidegrees_celsius":[],"bounded_capture":null,"configure":null,"firmware_idle_cleanup":null,"firmware_programmed":false,"hardware_enumerated":false,"info":null,"running_status":null,"start":null,"stop":null,"valid_job_ids":["1debfc21-6999-494f-9c36-1995990e8f36","b65cacb4-1161-424f-a03c-1ad38b85c272"]} | The board never enumerated, so the firmware lifecycle did not start. | ["physical-campaign"] |
| stream_health | Sequence gaps, loss, parser errors, transport errors, host-queue drops, and unexplained firmware errors are zero unless a named negative case declares and reconciles them. | **INCONCLUSIVE** | true | {"acquisition_service_utilization_ratio":null,"adc_pair_rate_hz":null,"cache_errors":null,"command_latency_maximum_milliseconds":null,"command_latency_p99_milliseconds":null,"complete_frame_loss":null,"dma_errors":null,"gpio_sample_rate_hz":null,"measurement_duration_seconds":null,"parser_errors":null,"sequence_gaps":null,"transport_errors":null,"trigger_errors":null,"usb_service_utilization_ratio":null} | No physical stream began, so rates, loss, and error counters were not observed. | ["physical-campaign"] |
| counter_conservation | All applicable frame, item, payload-byte, framed-byte, queue, loss, and source-stage equations are exact and nonsaturated. | **INCONCLUSIVE** | true | {"equations":null,"firmware_counters":null,"host_counters":null} | No physical counters were emitted because the runner never started. | ["physical-campaign"] |
| queue_bounds | Every observed queue depth and high-water value is at or below its same-artifact advertised capacity. | **INCONCLUSIVE** | {"packet_capacity_frames":200} | {"host_high_water":null,"packet_high_water_frames":null,"service_queue_depth_after_attempts":0} | Firmware and host acquisition queues were never instantiated for a run. | ["physical-campaign"] |
| final_idle_cleanup | STOP and final cleanup leave IDLE with every required ownership and transport gauge at zero. | **INCONCLUSIVE** | true | {"firmware_gauges":null,"firmware_idle_confirmed":null,"firmware_stop_attempted":false,"service_queue_depth":0,"service_recovered_after_each_attempt":true} | No firmware session existed to STOP or query for IDLE; only service recovery was observable. | ["physical-campaign"] |
